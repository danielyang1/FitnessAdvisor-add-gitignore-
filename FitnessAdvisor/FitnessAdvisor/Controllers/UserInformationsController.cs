...
namespace FitnessAdvisor.Controllers
{
    public class UserInformationsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "ID,UserID,firstName,lastName,age,weight,height,sex")] UserInformation userInformation)
        {
            if (ModelState.IsValid)
            {
                userInformation.UserID = User.Identity.GetUserId();

                var alreadyEnteredInfo = db.UserInformations.Where(u => u.UserID == userInformation.UserID).FirstOrDefault();

                if (alreadyEnteredInfo != null)
                {
                    db.UserInformations.RemoveRange(db.UserInformations.Where(s => s.UserID == userInformation.UserID));
                    db.SaveChanges();
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,UserID,firstName,lastName,age,weight,height,sex")] UserInformation userInformation)
        {
            if (ModelState.IsValid)
            {
